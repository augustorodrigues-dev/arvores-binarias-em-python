...
class Jogo:
    def __init__(self):
        ...
        self.intro_text = [
            ...
            "> [A] Auto (Lento) | [F] Fast Fill (Rápido) | [M] Mix",
            "> [I] Inserir | [X] Remover | [B] Buscar | [R] Reset",
    ...
    def _atualizar_layout(self):
        ...
        if self.fase in (1, 2, 4):
            def layout_bin(nid, depth, x_min, x_max):
                ...
                l = self.estrutura.tree._get_left(nid) if self.fase != 2 else self.estrutura.tree._left(nid)
                r = self.estrutura.tree._get_right(nid) if self.fase != 2 else self.estrutura.tree._right(nid)
            ...
            def mapear_todos(nid):
                ...
                node = self.estrutura.tree.nodes[nid]
                px, py = node.point
                sx = OFFSET_X + (px / 100) * L_UTIL; sy = OFFSET_Y + (py / 100) * A_UTIL
            ...
            mapear_todos(self.estrutura.tree.root)
             ...
             def layout_234(nid, depth, x_min, x_max):
                ...
                if node.leaf or not node.children:
                    ...
                    child_centers = []
                    width = x_max - x_min; span = max(width // n_children, 80); cur_x = x_min
                    for child_id in node.children:
                        layout_234(child_id, depth + 1, cur_x, cur_x + span)
                        if child_id in self.node_positions_234: child_centers.append(self.node_positions_234[child_id][0])
                        cur_x += span
