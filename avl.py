...
class AVLTreeAdj:
    ...
    def _height(self, nid: Optional[int]) -> int:
        if nid is None:
            return 0
        return self.nodes[nid].height

    def _update_height(self, nid: int) -> None:
        l = self._get_left(nid)
        r = self._get_right(nid)
    ...
    def _min_value_node(self, nid: int) -> int:
        cur = nid
        while self._get_left(cur) is not None:
            cur = self._get_left(cur)
        return cur
    ...
    def _remove_rec(self, nid: Optional[int], key: int) -> Optional[int]:
        ...
        node = self.nodes[nid]
        if key < node.key:
            self._set_left(nid, self._remove_rec(self._get_left(nid), key))
        elif key > node.key:
            self._set_right(nid, self._remove_rec(self._get_right(nid), key))
        else:
            if node.freq > 1:
                node.freq -= 1
                return nid
