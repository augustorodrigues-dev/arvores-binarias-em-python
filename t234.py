...
class Tree234:
    ...
    def insert(self, key: int) -> None:
        ...
        if len(root.keys) == 3:
            ...
            left_keys = [old_root.keys[0]]
            right_keys = [old_root.keys[2]]

            if old_root.leaf:
                left_children = []
                right_children = []
            else:
                left_children = old_root.children[:2]
                right_children = old_root.children[2:]
    ...
    def _split_child(self, parent_id: int, index: int) -> None:
        ...
        left_keys = [child.keys[0]]
        right_keys = [child.keys[2]]

        if child.leaf:
            left_children = []
            right_children = []
        else:
            left_children = child.children[:2]
            right_children = child.children[2:]
